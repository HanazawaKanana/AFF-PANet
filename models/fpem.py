...
from .cgblock import ContextGuidedBlock as cg
from .fusion import AFF
from .odconv import ODConv2d
from .SKAttention import CBAMBlock
...
class FPEM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.CG=cg(128,128)
        self.aff=AFF(128,8)
        self.cbam=TripletAttention(64)##CBAMBlock(128)

    def _upsample_add(self, x, y):
        _, _, H, W = y.size()
        return F.interpolate(x, size=(H, W), mode='bilinear') + y
        ##return self.aff(F.interpolate(x, size=(H, W), mode='bilinear'),y)

    def _upsample_aff(self, x, y):
        _, _, H, W = y.size()
        ##return F.interpolate(x, size=(H, W), mode='bilinear') + y
        return self.aff(F.interpolate(x, size=(H, W), mode='bilinear'),y)
    ...
    def forward(self, f1, f2, f3, f4):
        ##f4=self.CG(f4)
        f3 = self.smooth_layer3_1(self.dwconv3_1(self._upsample_aff(f4, f3)))##以下三个原来都是_upsample_add
        f2 = self.smooth_layer2_1(self.dwconv2_1(self._upsample_aff(f3, f2)))
        f1 = self.smooth_layer1_1(self.dwconv1_1(self._upsample_aff(f2, f1)))
        ...
        ##f1,f2,f3,f4=f1+self.cbam(f1),f2+self.cbam(f2),f3+self.cbam(f3),f4+self.cbam(f4)

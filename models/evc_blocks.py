...
##1*1卷积层
class Conv_BN_ReLU(nn.Module):##1x1卷积模块
    def __init__(self, in_planes, out_planes, kernel_size=1, stride=1, padding=0):
        ...
        for m in self.modules():##初始化模块的权重
...
class MSF(nn.Module):
    def __init__(self):
        ...
        ##self.conv1=Conv_BN_ReLU(64,128)
        self.conv2=Conv_BN_ReLU(128,128)
        self.conv3=Conv_BN_ReLU(256,128)
        self.conv4=Conv_BN_ReLU(512,128)
        self.pool=nn.AdaptiveAvgPool2d(20)
        self.bn = nn.BatchNorm2d(128)
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv2d(512, 128, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self,x1,x2,x3,x4):
        ...
        x4=self.conv4(x4)
        x3=self.conv3(x3)+F.interpolate(x4, size=x3.shape[2:], mode='bilinear', align_corners=False)
        x2=self.conv2(x2)+F.interpolate(x3, size=x2.shape[2:], mode='bilinear', align_corners=False)
        return  self.bn(x2)
        ##return F.sigmoid(x1+x2+x3+x4)
                          
if __name__=="__main__":
    x1=torch.randn(8,64,160,160)
    x2=torch.randn(8,128,80,80)
    x3=torch.randn(8,256,40,40)
    x4=torch.randn(8,512,20,20)
    model1=MSF()
    x=model1(x1,x2,x3,x4)
    model=EVCBlock(128,128)
    output=model(x)
    print(output.size())
    
    from thop import profile
    ##flops, params = profile(model, inputs=(x,))##这个函数可以计算FLOPs和参数量
    flops, params = profile(model1, inputs=(x1,x2,x3,x4))##这个函数可以计算FLOPs和参数量

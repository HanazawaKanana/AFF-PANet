...
class ScaleSpatialAttention(nn.Module):
    def __init__(self, in_planes, num_features, init_weight=True):
...
class RFBblock(nn.Module):
    def __init__(self,in_ch,residual=False):
        ...
        self.branch_1 = nn.Sequential(
                    nn.Conv2d(in_channels=in_ch, out_channels=inter_c, kernel_size=1, stride=1, padding=0),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=(1,3), stride=1, padding=(0,1)),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=3, stride=1, dilation=3, padding=3)
                    )
        self.branch_2 = nn.Sequential(
                    nn.Conv2d(in_channels=in_ch, out_channels=inter_c, kernel_size=1, stride=1, padding=0),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=(3,1), stride=1, padding=(1,0)),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=3, stride=1, dilation=3, padding=3)
                    )
        self.branch_3 = nn.Sequential(
                    nn.Conv2d(in_channels=in_ch, out_channels=inter_c, kernel_size=1, stride=1, padding=0),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=3, stride=1, padding=1),
                    nn.Conv2d(in_channels=inter_c, out_channels=inter_c, kernel_size=3, stride=1, dilation=5, padding=5)
                    )
        self.residual= residual
        self.bn=nn.BatchNorm2d(inter_c)
        self.attention=ScaleSpatialAttention(128,4)##注意力模块

    def forward(self,x):
        x_0 = self.bn(self.branch_0(x))
        x_1 = self.bn(self.branch_1(x))
        x_2 = self.bn(self.branch_2(x))
        x_3 = self.bn(self.branch_3(x))
        out = torch.cat((x_0,x_1,x_2,x_3),1)
        ##score=self.attention(out) 
        ##out = torch.cat((x_0*score[:, 0:1],x_1*score[:, 1:2],x_2*score[:, 2:3],x_3*score[:, 3:]),1)
        if self.residual:
            out +=x 
        return out

class ASFFmobile(nn.Module):##轻量版的ASFF
    def __init__(self, level, rfb=False, vis=False):
        ...
        self.inter_dim=128
        if level==0:
            self.stride_level_1 = add_conv(128, self.inter_dim, 3, 2, leaky=False)
            self.stride_level_2 = add_conv(128, self.inter_dim, 3, 2, leaky=False)
            self.expand = add_conv(self.inter_dim, 128, 3, 1, leaky=False)
        elif level==1:
            self.compress_level_0 = add_conv(128, self.inter_dim, 1, 1, leaky=False)
            self.stride_level_2 = add_conv(128, self.inter_dim, 3, 2, leaky=False)
            self.expand = add_conv(self.inter_dim, 128, 3, 1, leaky=False)
        elif level==2:
            self.compress_level_0 = add_conv(128, self.inter_dim, 1, 1, leaky=False)
            self.compress_level_1 = add_conv(128, self.inter_dim, 1, 1, leaky=False)
            self.expand = add_conv(self.inter_dim, 128, 3, 1,leaky=False)
...
if __name__ == '__main__':
    model = ASFFmobile(level=1)
    print(model)

    x = torch.randn(1, 128, 20, 20)
    y = torch.randn(1, 128, 40, 40)
    z = torch.randn(1, 128, 80, 80)
    out = model(x,y,z)
    print(out.shape)
    
    model=RFBblock(128)
    x = torch.randn(8, 128, 20, 20)
